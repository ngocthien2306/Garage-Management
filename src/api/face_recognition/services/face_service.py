...
class FaceServices:
    ...
    def find_user(self, model, img_path, db_path, detector="opencv", enfore_detection=True, silent=False):
        if os.path.isdir(db_path) is not True:
            raise ValueError("Passed db_path does not exist!")
        
        img_size = (112, 112)
        file_name = f"representations.pkl"
        ...
        df = pd.DataFrame(representations, columns=["identity", "r100_representation", "plate_num"])
        ...
        for index, instance in df.iterrows():
            source_embedding = instance[f"r100_representation"]
            dist = euclid_distance(source_embedding, img_representation)
            distances.append(dist[0])

        df["r100_euclid"] = distances
        df = df.drop("r100_representation", axis=1)
        df = df[df[f"r100_euclid"] <= 1.5]
        df = df.sort_values(
            by=[f"r100_euclid"], ascending=True
